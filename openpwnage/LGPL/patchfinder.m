...
uint32_t find_lwvm_call_offset(uint32_t region, uint8_t* kdata, size_t ksize) {
    for (uint32_t i = 0; i < ksize; i += 2) {
        if (*(uint64_t*)&kdata[i] == 0xf010798044406da0 && *(uint32_t*)&kdata[i+0x8] == 0xd0060f01 && *(uint16_t*)&kdata[i+0xC] == 0x4620) {
            ...
            olog("[*] found lwvm_call_offset: 0x%08x\n", lwvm_call_offset);
...
uint32_t find_substrate1(uint32_t region, uint8_t* kdata, size_t ksize) {
    for (uint32_t i = 0; i < ksize; i++) {
...
uint32_t find_substrate2(uint32_t region, uint8_t* kdata, size_t ksize) {
    for (uint32_t i = 0; i < ksize; i++) {
        if (*(uint64_t*)&kdata[i] == 0x0000f8dabf1e2800 && *(uint32_t*)&kdata[i+0x8] == 0x0004f040) {
...
uint32_t find_vm_fault_enter_patch(uint32_t region, uint8_t* kdata, size_t ksize) {
    ...
    olog("[*] found vm_fault_enter_patch: 0x%08x\n", vm_fault_enter_patch);
...
uint32_t find_vm_map_enter_patch(uint32_t region, uint8_t* kdata, size_t ksize) {
    ...
    if (!insn) {
        ...
        olog("[*] found vm_map_enter_patch: 0x%08x\n", vm_map_enter_patch);
...
uint32_t find_sandbox_call_i_can_has_debugger(uint32_t region, uint8_t* kdata, size_t ksize) {
    ...
    olog("[*] found sandbox_call_i_can_has_debugger: 0x%08x\n", sandbox_call_i_can_has_debugger);
